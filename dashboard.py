...
class DatabaseManager:
    ...
    def update_exception_status(self, exception_id, new_status, action_by):
        """
        Updates the correction_status for a single exception entry.
        If status is 'Yes', it also logs the action and marks the entry as accepted.
        """
        conn = self._get_connection()
        try:
            with conn.cursor() as cursor:
                # --- MODIFIED --- Also sets is_accepted = TRUE when status is 'Yes'
                if new_status == 'Yes':
                    sql = "UPDATE exceptions SET correction_status = %s, is_accepted = TRUE WHERE id = %s"
    ...
    def batch_update_exception_status(self, exception_ids, new_status, action_by):
        """
        Updates the correction_status for a list of exception IDs.
        If status is 'Yes', it also logs the actions and marks entries as accepted.
        """
        ...
        try:
            with conn.cursor() as cursor:
                ...
                # --- MODIFIED --- Also sets is_accepted = TRUE when status is 'Yes'
                if new_status == 'Yes':
                    sql = f"UPDATE exceptions SET correction_status = %s, is_accepted = TRUE WHERE id IN ({placeholders})"
    ...
    def update_exception_status(self, exception_id, new_status, action_by):
        ...
        try:
            ...
            # --- NEW --- Log the action if corrected
            if new_status == 'Yes':
                self.log_correction_action(exception_id, action_by)
    ...
    def batch_update_exception_status(self, exception_ids, new_status, action_by):
        ...
        try:
            ...
            # --- NEW --- Log the actions if corrected
            if new_status == 'Yes':
                self.batch_log_correction_action(exception_ids, action_by)
...
def show_correction_entries_page():
    ...
    # --- 1. Get User Context and Permissions (No changes here) ---
    user_role = st.session_state.get("role")
    username = st.session_state.get("username_actual")
    managed_users = st.session_state.get("managed_users", [])
    ...
    # --- 2. Notifications (No changes here) ---
    ...
    # --- 3. Filters and Data Fetching (No changes here) ---
    st.markdown("---")
    st.markdown("#### Filters")
    ...
    if all_accessible_entries.empty:
        st.success("✅ No correction entries found for your current view.")
        return
    ...
    if narration_filter:
        try:
            ...
            pass # Ignore errors if narration column is not string type
    ...
    # --- 4. Bulk Status Update (No changes here) ---
    # ... (This section remains unchanged) ...
    st.markdown("---")
    st.markdown("#### Bulk Update Status")
    st.caption("Set the status for all currently visible entries at once.")
    
    bulk_col1, bulk_col2, bulk_col3 = st.columns(3)
    if bulk_col1.button("Set All to 'Yes'", use_container_width=True):
        with st.spinner("Updating all entries..."):
            db_manager.batch_update_exception_status(entries_df['id'].tolist(), 'Yes', action_by=username)
        st.rerun()

    if bulk_col2.button("Set All to 'No'", use_container_width=True):
        with st.spinner("Updating all entries..."):
            db_manager.batch_update_exception_status(entries_df['id'].tolist(), 'No', action_by=username)
        st.rerun()
        
    if bulk_col3.button("Set All to 'Pending'", use_container_width=True):
        with st.spinner("Updating all entries..."):
            db_manager.batch_update_exception_status(entries_df['id'].tolist(), 'Pending', action_by=username)
        st.rerun()


    st.markdown("---")

    # --- 5. Display Individual Entries (This section contains the fix) ---
    for _, row in entries_df.iterrows():
        exception_id = row['id']
        current_status = row['correction_status']
        
        # --- MODIFIED --- Using database column names (lowercase) instead of original Excel names
        with st.expander(f"**ID: {exception_id}** | User: **{row.get('created_user')}** | Dept: **{row.get('department')}** | Status: **{current_status}**"):
            ...
            with action_col1:
                ...
                if new_status != current_status:
                    db_manager.update_exception_status(exception_id, new_status, action_by=username)
                    st.rerun()
            with action_col2:
                if show_accept_button:
                    st.write("**Resolve Entry:**")
                    if st.button("Accept and Close", key=f"accept_btn_{exception_id}", type="primary", use_container_width=True):
