        ...
        if dept == "Parent Seed":
            ...
            if sub_dept == "Sales Brand":
                if vertical == "FC-field crop":
                    ...
                    if self.is_blank(zone):
                        reasons.append("Need to update Zone can not left Blank")
                    elif zone not in self.ref_files.get("SaleFC_Zone", []):
                        reasons.append("Incorrect Zone Name for FC-field crop Vertical")
                    if self.is_blank(region):
                        reasons.append("Need to update Region Name can not left Blank")
                    elif region not in self.ref_files.get("SBFC_Region", []):
                    ...
                    if self.is_blank(zone):
                        reasons.append("Need to update Zone can not left Blank")
                    elif zone not in self.ref_files.get("SaleVC_Zone", []):
                        reasons.append("Incorrect Zone Name for VC-Veg Crop Vertical")
                    if self.is_blank(region):
                        reasons.append("Need to update Region Name can not left Blank")
                    elif region not in self.ref_files.get("SBVC_Region", []):
                    ...
                    if self.is_blank(zone):
                        reasons.append("Need to update Zone can not left Blank")
                    elif zone not in self.ref_files.get("SaleRS_Zone", []):
                        reasons.append("Incorrect Zone Name for Root Stock Crop Vertical")
                    if self.is_blank(region):
                        reasons.append("Need to update Region Name can not left Blank")
                    elif region not in self.ref_files.get("SBRS_Region", []):
...
def show_user_performance_page(start_date, end_date):
    ...
    if selected_user_for_perf == "All Users (Summary)":
        ...
        display_metric("Validation Runs in Scope", f"{len(run_ids_in_scope)}", container=kpi_col3)
    ...
    # --- Single User View ---
        ...
        avg_rate = user_perf_df['exception_rate'].mean()
        runs_involved = user_perf_df['run_id'].nunique()
        display_metric("Total Exceptions", f"{int(total_exc):,}", container=kpi_col1)
        display_metric("Avg. Exception Rate", f"{avg_rate:.2f}%", container=kpi_col2)
        display_metric("Runs Involved", f"{runs_involved}", container=kpi_col3)
        ...
        if not user_perf_df.empty:
            ...
            user_perf_df['upload_time'] = pd.to_datetime(user_perf_df['upload_time'])
